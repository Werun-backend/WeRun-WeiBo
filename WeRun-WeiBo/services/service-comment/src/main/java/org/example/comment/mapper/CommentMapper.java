...
public interface CommentMapper {
    ...
    @Select("select * from CTP where post_id=#{postId}")
    List<CTPVO> getCTP(String postId);
    @Select("select * from CTC where posted_comment_id = #{commentId} and status = 1")
    ...
    @Select("select * from CTP where post_id = #{postId} order by is_liked")
