...
public class PushServiceImpl implements PushService {
    ...
    //查询所有帖子并分页展示
    @Override
    public PageResult<PostBO> pushPosts(int page, int pageSize) {
        logger.debug("正在进行推送服务");
        List<PostBO> allPosts = pushPostMapper.selectAllPosts();
        logger.debug("推送数据获取完成");
        logger.debug("开始分页");
        int total = allPosts.size();
        int offset = (page - 1) * pageSize;
        int end = Math.min(offset + pageSize, total);
        List<PostBO> pagePosts = allPosts.subList(offset, end);
