...
public interface CommentMapper {
    ...
    @Select("select comment_id, post_id, user_id, content,status, reply_num,is_liked, create_time  from CTP where post_id=#{postId} and status = 1")
    ...
    @Select("select comment_id, post_id, user_id, content,status,reply_num,is_liked, create_time from CTP where post_id = #{postId} order by is_liked")
